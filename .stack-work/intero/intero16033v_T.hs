...
uploadmsg :: Socket -> String -> String -> Fileserver -> Integer -> DirectoryServer -> IO (   )
uploadmsg sock filename fdata fs rand        server@DirectoryServer{..}= do
  ...
  fm <- atomically $ newFilemapping filename rand (getFileserveraddress fs) (getFileserverport fs) getZonedTime
  atomically $ addFilemapping server filename rand (getFileserveraddress fs) (getFileserverport fs) getZonedTime
...
joinCommand sock server@DirectoryServer{..} command = do
  ...
  atomically $ addFileserver nodeID fs
...
incrementFileserverCount :: TVar Integer -> STM ()
