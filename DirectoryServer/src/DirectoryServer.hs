...
fsJoin fs@(FileServer key _ _) = liftIO $ do
...
openFile key = liftIO $ do
            ...
            manager <- newManager defaultManagerSettings
            res <- runClientM (download fileName file) (ClientEnv manager (BaseUrl Http (address file) (read(port file)) ""))
            case res of
