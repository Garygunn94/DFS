...
type LockingApi = 
    "lock" :> Capture "fileName" String :> Get Bool :<|>
    "unlock" :> Capture "fileName" String :> Get Bool :<|>
    "islocked" :> Capture "fileName" String :> Get Bool
...
lockFile fName = liftIO $ do
	...
	withMongoDbConnection $ do
        ...
        let locks = catMaybes $ DL.map (\ b -> fromBSON b :: Maybe Lock) docs
        case locks of
          [(Lock _ True)] -> return False
          [(Lock _ False)] -> liftIO $ do
              withMongoDbConnection $ upsert (select ["_id" =: fName] "LOCK_RECORD") $ toBSON $ (Lock fName True)
              return True
          [] -> liftIO $ do
              withMongoDbConnection $ upsert (select ["_id" =: fName] "LOCK_RECORD") $ toBSON $ (Lock fName True)
              return True
	...
	withMongoDbConnection $ do
        ...
        case locks of
          ...
          [(Lock _ True)] -> liftIO $ do
              withMongoDbConnection $ upsert (select ["_id" =: fName] "LOCK_RECORD") $ toBSON $ (Lock fName False)
              return True
          [] -> liftIO $ do
              withMongoDbConnection $ upsert (select ["_id" =: fName] "LOCK_RECORD") $ toBSON $ (Lock fName False)
              return True
